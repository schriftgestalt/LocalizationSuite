...
- (void)_emitKey:(NSString *)key value:(id)value leadingComment:(NSString *)leadingComment inlineComment:(NSString *)inlineComment {
	...
	if (_asReference && [self optionIsActive:BLFileInterpreterDeactivatePlaceholderStrings] && [self.ignoredPlaceholderStrings containsObject:[keyObject stringForLanguage:_language]])
		[keyObject setIsActive:NO];
...
- (void)autotranslateUsingObjects:(NSArray *)translations {
	...
	for (i = 0, j = 0; i < [objects count] && j < [translations count]; i++) {
		...
		while ([origValue compareAsString:[translation objectForLanguage:_language]] == NSOrderedDescending && j < [translations count] - 1)
			translation = [translations objectAtIndex:++j];

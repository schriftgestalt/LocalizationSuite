...
- (void)matchingThread
{
	...
	// Init
		...
		for (NSUInteger m=0, k=0; k<[_keyObjects count] && m<[_matchingObjects count]; k++) {
			...
			while (([targetString compareAsString: [matchKeyObject valueForKey: _matchLanguage]] == NSOrderedDescending
					|| ([matchKeyObject isEmptyForLanguage: _targetLanguage] && [matchKeyObject isEmptyForLanguage: baseTargetLanguage]))
				   && m<[_matchingObjects count]-1)

...
+ (NSArray *)getLengthWarningsFromOrginal:(NSString *)original andTranslation:(NSString *)translated
{
	...
	if (transLength - origLength > LTTranslationCheckerMinDeviationLength
		&& transLength > origLength * (1. + LTTranslationCheckerMaxLengthDeviation/100)) {
...
+ (NSArray *)getQuoteErrorsFromTranslation:(NSString *)translated andLanguage:(NSString *)language
{
	...
	for (i=0; i<[quotes count]; i++)
	{
		...
		for (j = 0; j<[quotingStyles count]; j++) {
			...
			if ((level%2 == 1 && [begin isEqual: [style objectForKey: @"begin"]] && [end isEqual: [style objectForKey: @"end"]])
				|| (level%2 == 0 && [begin isEqual: [style objectForKey: @"sndBegin"]] && [end isEqual: [style objectForKey: @"sndEnd"]]))
				break;
		}
